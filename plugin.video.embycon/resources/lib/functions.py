...
from .datamanager import DataManager, clear_cached_server_data, invalidate_content_cache
...
def _flag_fresh_refresh() -> None:
    """Ouvre une courte fenetre de fraicheur du cache apres un marquage.

    Pendant ~10 s, datamanager.get_items ignore le cache pour l'URL demandee.
    Le Container.Refresh qui suit re-interroge donc le serveur pour la liste
    OU le widget d'accueil actuellement focalise : l'element vu disparait (si
    hide_watched est actif) et le statut vu/non-vu est a jour -- sans recharger
    le skin ni ejecter l'utilisateur de sa page.
    """
    HomeWindow().set_property("embycon_fresh_until", str(time.time() + 10))
